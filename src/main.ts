...
import { AppEvents, categoryMap, API_URL, CDN_URL } from './utils/constants';
...
events.on(AppEvents.PRODUCTS_RECEIVED, (products: IProduct[]) => {
...
events.on(AppEvents.PRODUCT_SELECT, (data: { id: string }) => {
...
events.on(AppEvents.PRODUCT_SUBMIT, (data: { id: string }) => {
  const product = productsModel.getProductById(data.id)
...
events.on(AppEvents.PRODUCT_DELETE, (data: { id: string }) => {
  const product = productsModel.getProductById(data.id)
...
events.on(AppEvents.BASKET_PLACE_ORDER, () => {
...
events.on(AppEvents.FORM_PAYMENT_CHANGED, (data: { payment: TPayment }) => {
...
events.on(AppEvents.FORM_ADDRESS_CHANGED, (data: { address: string }) => {
...
events.on(AppEvents.FORM_ORDER_SUBMIT, () => {
...
events.on(AppEvents.FORM_EMAIL_CHANGED, (data: { email: string }) => {
...
events.on(AppEvents.FORM_PHONE_CHANGED, (data: { phone: string }) => {
...
events.on(AppEvents.BUYER_CHANGE, (data: { field: string }) => {
...
events.on(AppEvents.FORM_INPUT_FOCUS, () => {
  playSound(soundClick)
})
